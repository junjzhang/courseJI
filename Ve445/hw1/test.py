...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct  9 13:59:48 2019
...
def my_kel(a,b):
    ...
    return np.exp(-( np.ones(((a-b).shape[0],2))@np.abs(a-b).T )/2)
...
#train
#s1 = SoftMarginSVM(X_train,Y1_train)
##s2 = SVM(X_train,Y2_train)
##s3 = SVM(X_train,Y3_train)
#start = time.clock()
#s1.training(kernel="Laplace")
#elapsed = (time.clock()-start)
#print("Time:",elapsed)
#print(s1.testing(X_test,Y1_test))

model1 = svm.SVC(kernel="rbf",gamma = 0.5, degree = 10, tol = 0.00000000000001)
start2 = time.clock()
