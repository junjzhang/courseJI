  ...
  while (endFlag) {
    ...
    if (recvfrom(sockfd, msg, MAX_MESSAGE_SIZE, MSG_DONTWAIT,
      ...
      if (recvHeader.flag == ACK) {
        ...
        // resend if NACK
      } else if (recvHeader.flag == NACK) {
        payloadLen = ((recvHeader.seq == numSend - 1) && (divFlag > 0))
                         ? divFlag
                         : MAX_PAYLOAD_SIZE;
        sendto(sockfd,
               (void *)(buf + (recvHeader.seq - currentSeq) * sizeof(packet)),
               payloadLen + sizeof(header), 0,
               (const struct sockaddr *)&recvAddr, sizeof(struct sockaddr_in));
        struct SRHeader tempHeader;
        memcpy(&tempHeader,
               buf + (recvHeader.seq - currentSeq) * sizeof(packet),
               sizeof(header));
        logInfo(tempHeader, logfp);
      }
      // reset time
    ...
    while (FINflag) {
      ...
      if (recvfrom(sockfd, msg, MAX_MESSAGE_SIZE, MSG_DONTWAIT,
        ...
        if (recvHeader.flag == DATA) {
          ...
          // check for gap and send NACK
          unsigned int continueCnt = 1;
          unsigned int NACKFlag = 0;
          for (continueCnt = 0; continueCnt < windowSize; continueCnt++) {
            unsigned int temp;
            memcpy(&temp, buf + continueCnt * (8 + MAX_PAYLOAD_SIZE),
                   sizeof(temp));
            if (temp == 0)
              break;
          }
          if (continueCnt < windowSize) {
            if (continueCnt == 0) {
              if (!advanceFlag)
                NACKFlag = 1;
            } else
            ...
            if (NACKFlag) {
              ACKHeader.flag = NACK;
              ACKHeader.seq = currentACKSeq + continueCnt;
              ACKHeader.len = 0;
              ACKHeader.crc = crc32(NULL, 0);
              memcpy(ACKMsg, (char *)&ACKHeader, sizeof(ACKHeader));
              sendto(sockfd, (void *)ACKMsg, sizeof(ACKHeader), 0,
