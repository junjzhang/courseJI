...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [
            ...
            "*** YOUR CODE HERE ***"
            (x, y), cornerKeys = state
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)

            if not self.walls[nextx][nexty]:
                nextPosition = (nextx, nexty)
                nextCornerKeys = tuple([
                    cornerKey for cornerKey in cornerKeys
                    if self.corners[cornerKey] != nextPosition
                ])

                successors.append((action, 1, (nextPosition, nextCornerKeys)))
...
def cornersHeuristic(state, problem):
    ...
    for cornerKey in cornerKeys:
        distances.append(util.manhattanDistance(corners[cornerKey], state[0]))
...
def foodHeuristic(state, problem):
    ...
    for food in foodGrid.asList():
        subProblem = PositionSearchProblem(problem.startingGameState,
                                           start=position,
                                           goal=food,
                                           warn=False,
                                           visualize=False)
        distances.append(len(search.bfs(subProblem)))
    return max(distances)
