...
class ValueIterationTest(testClasses.TestCase):
    ...
    def executeNIterations(self, grades, moduleDict, solutionDict, n,
                           checkPolicy):
        testPass = True
        ...
        with open(filePath, 'w') as handle:
            ...
            for n in self.numsIterationsForDisplay:
                valuesPretty, qValuesPretty, actions, policyPretty = self.runAgent(
                    moduleDict, n)
                handle.write(
                    self.prettyValueSolutionString('values_k_%d' % n,
                                                   valuesPretty))
                for action in actions:
                    handle.write(
                        self.prettyValueSolutionString(
...
class ApproximateQLearningTest(testClasses.TestCase):
    def __init__(self, question, testDict):
        ...
        if sys.platform == 'win32':
            ...
            _, _, question_name, test_name = testDict['test_out_file'].split(
                '/')
...
class QLearningTest(testClasses.TestCase):
    def __init__(self, question, testDict):
        ...
        if sys.platform == 'win32':
            ...
            print(testDict['test_out_file'].split('/'), file=sys.stderr)
            _, _, question_name, test_name = testDict['test_out_file'].split(
                '/')
        self.experiences = Experiences(test_name.split('.')[0])
        if maxPreExperiences < numExperiences:
...
class EvalAgentTest(testClasses.TestCase):
    def __init__(self, question, testDict):
        ...
        self.nonTimeoutMinimum = int(
            testDict['nonTimeoutMinimum']
        ) if 'nonTimeoutMinimum' in testDict else None
...
class GridPolicyTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        if not hasattr(moduleDict['analysis'], self.parameterFn):
            self.addMessage('Method not implemented: analysis.%s' %
                            (self.parameterFn, ))
            return self.testFail(grades)
        ...
        for x in range(width):
            for y in range(height):
                if self.policy[x][y] in actionMap and policy[
                    (x, y)] != actionMap[self.policy[x][y]]:
                    differPoint = (x, y)
                    policyPassed = False

        if not policyPassed:
            self.addMessage('Policy not correct.')
            self.addMessage('    Student policy at %s: %s' %
